...
def getDailyUSDataYears(years = [], months = [], days = [], stations = []):
    ...
    print ("enter getDailyUSDataYears: ")
    for stationMonthCode in measurements.fileMeasurements.keys():
        try:
            stationMonth = measurements.fileMeasurements[stationMonthCode]
            
          
            if ((not years) or (stationMonth.year in years)) and ((not months) or (stationMonth.month in months)) and ((not stations) or (stationMonth.stationID in stations)):
                if (not days):
                    for x in range(1, 31):
                        days.append(str(x))
                for day in days:
